...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const getInstance = (obj) => {
      ...
      if (state.myChartInstance) {
        state.myChartInstance.on("finished", (e) => {
          if (!state.isFinished) {
            initChartTimer();
          }
        });
        ...
        // 鼠标悬停
        state.myChartInstance.on("mouseover", handleMouseOver);

        state.myChartInstance.on("mouseout", handleMouseOut);
      }
    };
    ...
    const initChartTimer = function () {
      const options = state.myChartInstance.getOption();
        ...
        series: [
          {
            ...
            radius: [
              fitChartSize(props.radius[0]),
              fitChartSize(props.radius[1]),
            ],
            label: {
              show: false,
              color: "inherit",
