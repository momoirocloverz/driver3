...
const sectionTitle = ref("dailyNumber");
...
const columnSources = reactive({
  dailyNumber: [
    ...
    { field: "management", name: "辖区网格员", width: "12.5%" },
    {
      field: "picture",
      name: "图片",
      width: "15%",
  ...
  censusVisits: [
    ...
    { field: "manager", name: "辖区网格员", width: "12.5%" },
    {
      field: "pict_url",
      name: "走访照片",
      width: "15%",
    ...
    { field: "visit_time", name: "走访时间", width: "20%" },
    { field: "times", name: "累计走访次数", width: "15%" },
  ],
});
request
  .get("/api/Person/getTodaySpecialPerson", { pageSize: 10000 })
  .then((result) => {
    dataSources.dailyNumber = result.data.data
...
request.get("/api/Person/getPersonnelDetail").then((result) => {
...
request.get("/api/Person/getPersonnelDetailList").then((result) => {
  dataSources.censusVisits = result.data.data
    .sort((a, b) => compareDateString(a.visit_time, b.visit_time))
    .map((e, idx) => ({
      ...e,
      _tid: `${Date.now()}-${idx}`,
      _idx: idx + 1,
