      ...
      <div v-for="(item, index) in list" :key="index" class="_item">
        ...
        <div class="_item-center">
          ...
          <div
            ...
            "
            :style="'width: calc(' + item.number / max + '*100%)'"
          ></div>
          <img
            :src="
              index % 3 == 0
            ...
            "
            :style="'left: calc(' + item.number / max + '*100% - 13px)'"
          />
        </div>
        <div class="_item-right text-over1">
          {{ item.number }}<span>人</span>
        </div>
      </div>
    </div>
...
import { getMonthOutpatientNumber } from "@/api/hospital/api";
export default {
  setup() {
    const list = ref([]);
    const total = ref(10);
    ...
    // 重新加载数据
    const reload = () => {
      getMonthOutpatientNumber().then((res) => {
        if (res.code == 0) {
          total.value = res.data.length;
          for (let item of res.data) {
            console.log(item);
            if (parseInt(item.number) > max.value) {
              max.value = parseInt(item.number);
            }
          }
          list.value = res.data;
          isLoad.value = true;
