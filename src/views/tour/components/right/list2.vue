      ...
      <div v-for="(item, index) in list" :key="index" class="_item">
        <div class="_item-left">
          ...
          <div
            ...
            :style="'width: calc(' + item.praise / maxGoods + '*100%)'"
          ></div>
          <div
            class="_item-left-title"
            :style="
              item.praise / maxGoods < 0.2
                ? 'right: 0px'
                : item.praise / maxGoods > 0.8
                ? 'right: calc(' + item.praise / maxGoods + '*100% - 40px)'
                : 'right: calc(' + item.praise / maxGoods + '*100% - 20px)'
            "
          >
            {{ item.praise }}条
          </div>
        </div>
        <div class="_item-center text-over1">{{ item.type }}</div>
        <div class="_item-right">
          <div class="_item-left-bg"></div>
          <div
            class="_item-left-current"
            :style="'width: calc(' + item.complaint / maxBad + '*100%)'"
          ></div>
          <!-- 这里做了判断，如果当前 item.complaint / maxBad 小于 0.2 那么直接默认为left: 0px -->
          <!-- 这里的0.1是根据当前总宽度 460px / 2 * 20% = 46px 所以这里就默认为0.2了 -->
          <!-- 这里的0.1是根据当前总宽度 460px / 2 * 20% = 46px 所以这里就默认为0.2了 -->
          <div
            class="_item-left-title"
            :style="
              item.complaint / maxBad < 0.2
                ? 'left: 0px'
                : item.complaint / maxBad > 0.8
                ? 'left: calc(' + item.complaint / maxBad + '*100% - 40px)'
                : 'left: calc(' + item.complaint / maxBad + '*100% - 20px)'
            "
          >
            {{ item.complaint }}条
          </div>
  ...
  setup(props) {
    ...
    const reload = () => {
      for (let item of props.data) {
        if (maxGoods.value < item.praise) {
          maxGoods.value = item.praise;
        }
        if (maxBad.value < item.complaint) {
          maxBad.value = item.complaint;
        }
      }
